...
use datafusion::{
    arrow::datatypes::{DataType, Field, Schema}, assert_batches_eq, error::Result, prelude::{col, lit, ParquetReadOptions, SessionContext}
};
...
async fn simple_dataframe_parse_sql_expr_demo() -> Result<()> {
    ...
    let df = ctx
        .read_parquet(
            &format!("data/alltypes_plain.parquet"),
